import { Box, Button, Flex, HStack, Input, InputGroup, InputRightElement, SimpleGrid, Text } from '@chakra-ui/react'
...
export default function Home() {
    ...
    useEffect(() => {
      ...
      const fetchPairs = (searchQuery) => {
        fetch(`https://api.dexscreener.com/latest/dex/search?q=${searchQuery}`)
          .then((res) => res.json())
          .then((res) => {
            setData(res.pairs.sort((a, b) => b.priceUsd - a.priceUsd));
          })
          .catch((err) => {
            // console.log(err);
          });
    ...
    return (
        <Box display={'flex'} gap={5} flexDir={{ base:'column',sm: 'column', md: 'column', lg: 'row' }}>
            <Box display={{ lg: "block", md: "none", sm: 'none', base: 'none' }}>
                <Sidebar />
            </Box>
            <Box  display={{ lg: "none", md: "flex", sm: 'flex', base: 'flex' }} flexDir={'row'} alignItems={'center'} justifyContent={'space-around'} mt={'10px'}>
                <HStack>
                    <MobileDrawer/> 
                    <Text className='pacifio' color={'#FFFFFF'} fontSize={'16px'} fontWeight={400}>NFTify</Text>
                </HStack>
                <Box>
                <Button className='poppins16' w={'83px'} h={'27.66px'} fontSize={'8.513px'} borderRadius={'20px'} background={'linear-gradient(131deg, #7C0F35 0%, #581266 100%)'} color={'white'}>
                    Connect
                </Button>
                </Box>
            </Box>
            ...
            <Box w={['90%','90%','90%','80%']} m={'auto'}>
                <Flex mt={'27px'} mb={'15px'} alignItems={'center'} justifyContent={'space-around'}>
                    <InputGroup className='poppins18' w={['90%','90%','90%','435px']} h={'50px'} flexShrink={0}>
                        <Input borderRadius={'20px'} color={'white'} border={'1px solid #FFF'} background={'linear-gradient(132deg, rgba(124, 15, 53, 0.20) 0%, rgba(88, 18, 102, 0.20) 100%)'}
                            placeholder='Search' onChange={(e) => setQuery(e.target.value)} />
                        <InputRightElement>
                            <Search2Icon color={'white'} />
                        </InputRightElement>
                    </InputGroup>
                    <Button display={{ lg: "block", md: "none", sm: 'none', base: 'none' }} className='poppins16' w={'156px'} h={'52px'} borderRadius={'20px'} background={'linear-gradient(131deg, #7C0F35 0%, #581266 100%)'} color={'white'}>Connect</Button>
                </Flex>
                <Box height={'850px'}
                    overflowY={"scroll"}
                    css={{
                    ...
                    <Text className='worksans' fontWeight={600}>Token Search Results</Text>
                    {data?.map((el,i) => (
                        <SimpleGrid key={i} mt={2} columns={[1, 1, 2, 4]}>
                            <Box m={'auto'} w={'279px'} h={'188px'} mt={'10px'} borderRadius={'10px'} background={'#390554'}>
                                <Text className='poppins16' mt={'23px'} ml={'32px'} mb={'10px'}>Basic Info</Text>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Pair created at</Text>
                                    <Text>{el.pairCreatedAt ? el.pairCreatedAt : "NA"}</Text>
                                </HStack>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Symbol</Text>
                                    <Text>{el.baseToken.symbol ? el.baseToken.symbol : "NA"}</Text>
                                </HStack>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Dex ID</Text>
                                    <Text>{el.dexId ? '#' + el.dexId.substring(0, 5) : "NA"}</Text>
                                </HStack>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Pair Address</Text>
                                    <Text>{el.pairAddress ? '#' + el.pairAddress.substring(0, 4) : "NA"}</Text>
                                </HStack>
                                <Box w={'56px'} h={'56px'} bgColor={'#960252'} rounded={'full'} ml={'215px'}>
                            ...
                            <Box m={'auto'} w={'279px'} h={'188px'} mt={'10px'} borderRadius={'10px'} background={'#390554'}>
                                <Text className='poppins16' mt={'23px'} ml={'32px'} mb={'10px'}>Base Token</Text>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Name</Text>
                                    <Text>{el.baseToken.name ? el.baseToken.name : "NA"}</Text>
                                </HStack>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Symbol</Text>
                                    <Text>{el.baseToken.symbol ? el.baseToken.symbol : "NA"}</Text>
                                ...
                                <HStack className='poppins12' ml={'32px'}>
                                    ...
                                    <Text>{el.baseToken.address ? '#' + el.baseToken.address.substring(0, 4) : "NA"}</Text>
                                </HStack>
                                <Box w={'56px'} h={'56px'} mt={'15px'} bgColor={'#960252'} rounded={'full'} ml={'215px'}>
                                    <Flex h={'100%'} justifyContent={'center'} alignItems={'center'}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            ...
                            <Box m={'auto'} w={'279px'} h={'188px'} mt={'10px'} borderRadius={'10px'} background={'#390554'}>
                                <Text className='poppins16' mt={'23px'} ml={'32px'} mb={'10px'}>Quote Token</Text>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Name</Text>
                                    <Text>{el.baseToken.name ? el.baseToken.name : "NA"}</Text>
                                </HStack>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Symbol</Text>
                                    <Text>{el.quoteToken.symbol ? el.quoteToken.symbol : "NA"}</Text>
                                </HStack>
                                <HStack className='poppins12' ml={'32px'}>
                                    <Text w={'40%'}>Address</Text>
                                    <Text>{el.quoteToken.address ? '#' + el.quoteToken.address.substring(0, 4) : "NA"}</Text>
                                </HStack>
                                <Box w={'56px'} h={'56px'} mt={'15px'} bgColor={'#960252'} rounded={'full'} ml={'215px'}>
                                    <Flex h={'100%'} justifyContent={'center'} alignItems={'center'}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
